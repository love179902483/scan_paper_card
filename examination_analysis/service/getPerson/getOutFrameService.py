...
class GetOutFrame():
    '''
        picturePath: 扫描出来图片路径
        paperInfo: 纸张信息
        outFrameInfo: 外部轮廓信息
    '''
    def __init__(self, picturePath, paperInfo, outFrameInfo):
    ...
    def init(self):
        ...
        # self.__getAllTemp()
        # 重新计算获取A4大小比例的纸张
        self.__reshapeToStandardSingle()

        # 截取第一张图片的信息,通过仿射变换获取正矩形
        return self.__getThisOutFrame()
    ...
    def __reshapeToStandardSingle(self):
            thisImg = cv2.imread(self.picturePath)
            ...
            # resize图片
            self.tempPicture = cv2.resize(thisImg, thisImageReshape, interpolation=cv2.INTER_CUBIC)
    ...
    def __getThisOutFrame(self):
        ...
        for point in contours:
            ...
            # 计算本框框面积容差
            toleranceResultArea = util.evaluateTolerance(thisPointArea, self.standardOutFrameArea)
            # 计算本框框周长
            thisPointLength = cv2.arcLength(point,True)
            # 计算周长容差
            toleranceResultLength = util.evaluateTolerance(self.standardOutFrameLength, thisPointLength)
